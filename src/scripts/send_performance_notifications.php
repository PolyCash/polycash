...
if ($app->running_as_admin()) {
	...
	while ($db_game = $running_games->fetch()) {
		...
		$linear_bet_q = "SELECT gio.game_io_id, gio.colored_amount, gio.option_id, gio.is_coinbase, gio.is_resolved, gio.game_out_index, gio.contract_parts, t.tx_hash, ca.account_id, p.ref_block_id, p.ref_round_id, p.ref_coin_blocks, p.ref_coin_rounds, p.effectiveness_factor, p.effective_destroy_amount, p.destroy_amount, p.".$game->db_game['payout_weight']."s_destroyed, p.game_io_id AS parent_game_io_id, p.contract_parts AS total_contract_parts, io.spend_transaction_id, io.spend_status, ev.*, et.vote_effectiveness_function, et.effectiveness_param1, o.entity_id, o.effective_destroy_score AS option_effective_destroy_score, o.unconfirmed_effective_destroy_score, o.unconfirmed_votes, o.name AS option_name, o.event_option_index, ev.destroy_score AS sum_destroy_score, ev.effective_destroy_score AS sum_effective_destroy_score, p.votes, o.votes AS option_votes, ug.user_game_id, u.notification_email FROM addresses a JOIN address_keys ak ON a.address_id=ak.address_id JOIN currency_accounts ca ON ak.account_id=ca.account_id JOIN user_games ug ON ug.account_id=ca.account_id JOIN users u ON ug.user_id=u.user_id JOIN transaction_ios io ON a.address_id=io.address_id JOIN transactions t ON t.transaction_id=io.create_transaction_id JOIN transaction_game_ios gio ON io.io_id=gio.io_id LEFT JOIN transaction_game_ios p ON gio.parent_io_id=p.game_io_id JOIN options o ON gio.option_id=o.option_id JOIN events ev ON o.event_id=ev.event_id LEFT JOIN event_types et ON ev.event_type_id=et.event_type_id WHERE gio.game_id=:game_id AND gio.is_coinbase=1 AND gio.resolved_before_spent=1 AND ug.notification_preference='email' AND u.notification_email LIKE '%@%' AND ev.payout_rule='linear' AND ev.event_starting_block <= ".$last_block_id." AND ev.event_payout_block >= ".$last_block_id." ORDER BY ug.user_game_id ASC, ev.event_index ASC;";
		...
		for ($bet_i=0; $bet_i<count($linear_bets); $bet_i++) {
			...
			if ($bet['user_game_id'] != $prev_user_game_id) {
				if ($prev_user_game_id !== false) {
					...
					$bet_summary .= "Your account is now worth <a href=\"".AppSettings::getParam('base_url')."/explorer/games/".$game->db_game['url_identifier']."/my_bets/?user_game_id=".$prev_user_game_id."\">".$app->format_bignum($unresolved_net_delta+($account_value/pow(10, $game->db_game['decimal_places'])))." ".$game->db_game['coin_name_plural']."</a>";
					
					$html_by_user_game[$prev_user_game_id] = "<p>".$bet_summary."</p><table>".$linear_table_header_html.$html_by_user_game[$prev_user_game_id]."</table>\n";

...
if ($app->running_as_admin()) {
	...
	if ($blockchain) {
		...
		$need_key_addresses = $app->run_query("SELECT * FROM addresses a WHERE a.is_mine=1 AND a.primary_blockchain_id=:blockchain_id AND NOT EXISTS (SELECT * FROM address_keys k WHERE k.address_id=a.address_id) LIMIT 1000;", ['blockchain_id' => $blockchain->db_blockchain['blockchain_id']]);

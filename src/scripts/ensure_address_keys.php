...
if ($app->running_as_admin()) {
	...
	if ($db_blockchain) {
		...
		$need_key_addresses = $app->run_query("SELECT * FROM addresses a WHERE a.is_mine=1 AND a.primary_blockchain_id=:blockchain_id AND NOT EXISTS (SELECT * FROM address_keys k WHERE k.address_id=a.address_id) LIMIT 1000;", ['blockchain_id' => $db_blockchain['blockchain_id']]);
		...
		if ($need_key_addresses->rowCount() > 0) {
			...
			while ($address = $need_key_addresses->fetch()) {
				$new_key_q .= "(".$address['address_id'].", NULL, ".$app->quote_escape($save_method).", ".$app->quote_escape($address['address'])."), ";
			}

...
while ($db_running_game = $running_games->fetch()) {
	...
	if (!empty($running_game->db_game['auto_stake_featured_strategy_id']) && $running_game->last_block_id() == $blockchain->last_block_id()) {
		...
		foreach ($notify_users as $user_game) {
			...
			if ($user_game['account_id'] > 0 && (empty($user_game['latest_speedup_reminder_time']) || $user_game['latest_speedup_reminder_time'] < time() - (3600*24*5))) {
	...
	if ($db_running_game['every_event_bet_reminder_minutes'] > 0) {
		...
		foreach ($running_game->current_events as $current_event) {
			...
			if ($current_event->db_event['event_index']%6 == 0 && $seconds_left > 0 && $seconds_left <= $db_running_game['every_event_bet_reminder_minutes']*60) {
				...
				$notify_users = $app->run_query("SELECT ug.*, u.notification_email, u.user_id FROM user_games ug JOIN users u ON ug.user_id=u.user_id WHERE ug.notification_preference='email' AND u.notification_email LIKE '%@%' AND ug.game_id=:game_id GROUP BY ug.user_id;", ['game_id' => $running_game->db_game['game_id']]);
				...
				while ($notify_user = $notify_users->fetch()) {
					...
					if ($sec_since_last_notification > 60*$db_running_game['every_event_bet_reminder_minutes']) {
						...
						$claim_amount_int = Faucet::getMaxClaimAmount($app, $notify_user['user_id'], $running_game);
						
						$faucet_txt = "";
						
						if ($claim_amount_int > 0) {
							$faucet_txt .= "You're eligible to <a href=\"".AppSettings::getParam('base_url').'/wallet/'.$running_game->db_game['url_identifier']."/\">claim ".$running_game->display_coins($claim_amount_int)."</a> from the faucet.\n";
						}
						...
						if (!empty($faucet_txt)) $message_html .= $faucet_txt."<br/><br/>\n";

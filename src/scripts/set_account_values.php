...
if ($app->running_as_admin()) {
	...
	while ($db_game = $all_games->fetch()) {
		...
		$user_games = $app->run_query("SELECT * FROM users u JOIN user_games ug ON u.user_id=ug.user_id WHERE ug.game_id=:game_id ORDER BY u.user_id ASC;", ['game_id' => $game->db_game['game_id']]);
		...
		while ($user_game = $user_games->fetch()) {
			...
			$app->run_query("UPDATE user_games SET account_value=:account_value WHERE user_game_id=:user_game_id;", [
				...
				'user_game_id' => $db_user['user_game_id']
			]);

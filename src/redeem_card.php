...
$peers = $app->run_query("SELECT * FROM peers WHERE is_public_card_issuer=1 ORDER BY peer_name ASC;")->fetchAll();
	...
	<div class="container-fluid">
		<div class="panel panel-info" style="margin-top: 15px;">
			<div class="panel-body">
				Sorry, <?php echo AppSettings::getParam('site_name'); ?> cards are not enabled on this node.
			</div>
		</div>
	</div>
	<?php
		...
		if ($uri_parts[1] == "redeem") {
			...
			if (!empty($card_id) && !empty($peer_id)) {
				...
				if ($card) {
					...
					<div class="panel panel-info" style="margin-top: 15px;">
						<div class="panel-body">
							Sorry, no card was found matching that ID number.
						</div>
					</div>
					<?php
				}
					...
					<div class="row">
						<div class="col-sm-6 col-sm-push-3 text-center">
							...
							<form action="/redeem/" method="get" onsubmit="thisPageManager.search_card_id(); return false;">
								<div class="form-group">
									...
									<select class="form-control" name="peer_id" id="peer_id">
										...
										foreach ($peers as $peer) {
											echo "<option value=\"".$peer['peer_id']."\">".$peer['peer_name']."</option>\n";
										}
										?>
									</select>
								</div>
								...
								<input class="btn btn-success" type="submit" value="Find my card" onclick="thisPageManager.search_card_id(); return false;" />
							</form>
							<br/>
						</div>
					</div>

	...
	window.onload = function() {
		...
		if ($_REQUEST['action'] == "start_bet") {
			...
			$(document).ready(function() {
				var show_event_pos = games[0].event_index_to_event_pos(<?php echo (int)$_REQUEST['event_index']; ?>);
				console.log("Initialize with a bet, event pos: ", show_event_pos);
		...
		if ($game->db_game['module'] == "MonsterDuels" && $being_determined_event && $base_options_formatted) {
			...
			thisMonsterDuelsManager = new MonsterDuelsManager(<?php echo $game->db_game['game_id']; ?>, '<?php echo $game->db_game['url_identifier']; ?>', <?php echo $being_determined_event->db_event['event_index']; ?>, <?php echo json_encode($base_options_formatted, JSON_PRETTY_PRINT); ?>, 0, <?php echo $app->coins_per_vote($game->db_game); ?>, <?php echo json_encode(array_intersect_key($being_determined_event->db_event, [
			...
			<div class="row">
				<div class="col-sm-6">
					...
					<div id="wallet_text_stats" style="display: block;">
						<?php
						echo $thisuser->wallet_text_stats($game, $blockchain_current_round, $blockchain_last_block_id, $blockchain_block_within_round, $mature_balance, $unconfirmed_amount+$immature_amount, $user_votes, $votes_value, $user_pending_bets, $user_game, $early_resolved_amount);
						?>
					</div>
				</div>
				<?php

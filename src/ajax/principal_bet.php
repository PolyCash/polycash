...
if ($thisuser && $game && $app->synchronizer_ok($thisuser, $_REQUEST['synchronizer_token'])) {
	if (isset($_REQUEST['amount']) && (float) $_REQUEST['amount'] == $_REQUEST['amount'] && $_REQUEST['amount'] > 0) {
		...
		if ($option) {
			...
			if ($address) {
				...
				$app->output_message(5, "Error, no address found for this option. Please try again.", false);
			}
		}
		else $app->output_message(4, "Error, invalid option ID.", false);
	}

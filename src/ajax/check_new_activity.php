...
if (!empty($_REQUEST['include_betting_events']) && $_REQUEST['include_betting_events'] == "true") $include_betting_events = true;
else $include_betting_events = false;

if (!empty($_REQUEST['include_being_determined_events']) && $_REQUEST['include_being_determined_events'] == "true") $include_being_determined_events = true;
else $include_being_determined_events = false;
...
if ($game->db_game['module'] == "MonsterDuels" && $game_loop_index >= 4) {
	...
	if ($being_determined_event && (string) $being_determined_event->db_event['event_index'] !== (string) $_REQUEST['md_bde_index']) {
		...
		$output['monsterduels_js'] .= "thisMonsterDuelsManager = new MonsterDuelsManager(".$game->db_game['game_id'].", '".$game->db_game['url_identifier']."', ".$being_determined_event->db_event['event_index'].", ".json_encode($base_options_formatted).", false, ".$app->coins_per_vote($game->db_game).", ".json_encode(array_intersect_key($being_determined_event->db_event, [

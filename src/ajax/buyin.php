...
if ($thisuser && $game && $app->synchronizer_ok($thisuser, $_REQUEST['synchronizer_token'])) {
	...
	if ($user_game) {
		...
		$buyin_currencies = $app->run_query("SELECT * FROM currencies c JOIN blockchains b ON c.blockchain_id=b.blockchain_id WHERE b.p2p_mode='rpc' ORDER BY c.name ASC;")->fetchAll();
		...
		if ($exchange_rate > 0) {
			if (!empty($_REQUEST['action']) && $_REQUEST['action'] == "check_amount") {
				...
				if (!empty($_REQUEST['invoice_id'])) {
					...
					if (isset($invoice) && $invoice['pay_currency_id'] != $buyin_currency['currency_id']) {
						$invoice = null;
						$invoice_error_message = "That invoice does not match your deposit currency.";
					}
				}
				...
				if (empty($invoice)) {
					if ($pay_to_account) {
						...
						if (!$invoice) $invoice_error_message = 'Failed to generate a deposit address.';
					}
					else $invoice_error_message = "Failed to load your deposit account.";
				}
				...
				else $content_html .= '<p class="text-danger">'.$invoice_error_message.'</p>';
			}
			else if (!empty($_REQUEST['action']) && $_REQUEST['action'] == "refresh") {}
			else {
				$content_html .= $app->render_view('buyin_currency_selector', [
					'game' => $game,
			...
			$content_html .= '<p class="text-danger">Failed to identify the exchange rate between '.$buyin_currency['short_name_plural'].' and '.$game->db_game['coin_name_plural'].'. Please do not send '.$buyin_currency['short_name_plural'].' right now.</p>';
		}

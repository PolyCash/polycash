...
if ($thisuser && $game) {
	...
	$featured_strategy = $app->run_query("SELECT * FROM featured_strategies WHERE featured_strategy_id=".$featured_strategy_id.";")->fetch();
	
	if ($featured_strategy && $featured_strategy['game_id'] == $game->db_game['game_id']) {
		$app->run_query("UPDATE user_strategies SET voting_strategy='featured', featured_strategy_id='".$featured_strategy['featured_strategy_id']."' WHERE strategy_id='".$user_strategy['strategy_id']."';");

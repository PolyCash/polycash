...
if ($game->db_game['inflation'] == "exponential") {
	...
	if ($event->db_event['payout_rule'] == "binary") {
		...
		if ($two_sided_contract_price > 0) {
			...
			echo "<p>".$app->format_bignum($confirmed_equivalent_contracts, false)." ".$event->db_event['track_name_short']." issued at $".$app->format_bignum($two_sided_contract_price, false)." per contract";
...
if ($event->db_event['payout_rule'] == "linear") {
	...
	if ($event_effective_coins > 0) {
		...
		$our_buy_price_round = $app->round_to($our_buy_price, 0, EXCHANGE_RATE_SIGFIGS, false);

...
$allowed_params = ['api_key', 'hours', 'force', 'amount_mode', 'aggressiveness'];
...
if ($user_game) {
	...
	$sec_between_applications = 60*60*($_REQUEST['hours'] ?? 40);
				...
				if ($coins_per_event > 0) {
					...
					foreach ($db_events as $db_event) {
						...
						foreach ($options_by_event as $option) {
							...
							if ($this_address) {
								$thisbet_io_amount = floor($io_nondestroy_per_event/$num_options_this_event);

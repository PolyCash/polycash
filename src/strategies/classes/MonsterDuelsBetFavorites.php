...
class MonsterDuelsBetFavorites {
	...
	public function run(array $runParams = []) {
		...
		$underdog_options = $this->app->run_query("SELECT * FROM options op JOIN entities en ON op.entity_id=en.entity_id WHERE op.event_id=:event_id ORDER BY en.hp DESC, RAND() LIMIT ".$num_options.";", ['event_id'=>$db_event['event_id']])->fetchAll();

		if (count($underdog_options) == 0) {
			return [10, "No options were selected to bet on."];
		}

		$num_txos = count($underdog_options)*2 + 2 + count($io_ids);
		...
		foreach ($underdog_options as $option) {
			$this_address = $this->app->fetch_addresses_in_account($account, $option['option_index'], 1)[0];

			if ($this_address) {
				$thisbet_io_amount = floor($io_nondestroy_per_event/count($underdog_options));

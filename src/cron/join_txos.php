...
if ($app->running_as_admin()) {
	...
	if ((empty($next_join_txos_time) || time() >= $next_join_txos_time) || !empty($_REQUEST['force'])) {
		if (!$process_locked || !empty($_REQUEST['force'])) {
			...
			$accounts = $app->run_query("SELECT ca.*, us.transaction_fee FROM currency_accounts ca JOIN user_games ug ON ca.account_id=ug.account_id JOIN user_strategies us ON ug.strategy_id=us.strategy_id WHERE ca.game_id IS NOT NULL AND ca.join_txos_on_quantity IS NOT NULL ORDER BY ca.account_id ASC;")->fetchAll();
			
			echo "Checking ".count($accounts)." accounts.\n";

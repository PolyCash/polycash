...
function refresh_mature_io_btns() {
	for (var i=0; i<mature_ios.length; i++) {
		var select_btn_text = "";
		var votes;
		if (games[0].payout_weight == 'coin') votes = mature_ios[i].amount;
		else if (games[0].payout_weight == 'coin_round') votes = (games[0].block_to_round(1+games[0].last_block_id) - games[0].block_to_round(mature_ios[i].create_block_id))*mature_ios[i].amount;
		else votes = (1 + games[0].last_block_id - mature_ios[i].create_block_id)*mature_ios[i].amount;
...
function load_plan_option_games() {
...
// OBJECT: Event
var Event = function(game, event_id, num_voting_options, vote_effectiveness_function) {
	this.game = game;
	this.game_event_index = game.events.length;
	...
	this.setVotingOptions = function(optionData) {
		this.votingaddr_count = optionData.length;
	...
	this.start_vote = function(option_id) {
		...
		$('#game'+this.game.instance_id+'_event'+this.game_event_index+'_vote_details_'+option_id).html($('#vote_details_general').html());
					...
					if (_this.game.refresh_page == "wallet" && result == "0") {
						...
						if (json_result['event_loop_index'] > _this.last_event_loop_index_applied) {
							if (json_result['new_block'] == "1") {
								...
								if (_this.game.refresh_page == "wallet") {
									if ((_this.game.last_block_id+1)%_this.game.game_round_length == 0) {
										...
										$('#'+_this.game.instance_id+'_event'+_this.game_event_index+'_vote_popups_disabled').slideDown('fast');
									}
									else {
							...
							if (parseInt(json_result['new_transaction']) == 1) {
								...
								if (user_logged_in) $('#game'+_this.game.instance_id+'_vote_details_general').html(json_result['vote_details_general']);
							}
							if (parseInt(json_result['new_block']) == 1 || parseInt(json_result['new_transaction']) == 1) {
								...
								for (var option_id=1; option_id<=_this.num_voting_options; option_id++) {
									$('#game'+_this.game.instance_id+'_event'+_this.game_event_index+'_vote_option_details_'+option_id).html(vote_option_details[option_id]);
								}
...
// OBJECT: Game
var Game = function(game_id, last_block_id, last_transaction_id, my_last_transaction_id, mature_io_ids_csv, payout_weight, game_round_length, min_bet_round, fee_amount, game_url_identifier, coin_name, coin_name_plural, refresh_page, event_ids) {
	...
	this.add_option_to_vote = function(option_id, name) {
		if (this.refresh_page != "wallet") {
			...
			if (games[0].option_has_votingaddr[option_id]) {
				...
				//$('#game'+this.instance_id+'_event'+this.game_event_index+'_vote_confirm_'+option_id).modal('hide');

...
if ($_REQUEST['key'] != "" && $_REQUEST['key'] == $GLOBALS['cron_key_string']) {
	...
	for ($real_game_i=0; $real_game_i<count($real_game_types); $real_game_i++) {
		if ($real_game_types[$real_game_i]->db_game['min_unallocated_addresses'] > 0) {
			...
			$q = "SELECT * FROM options WHERE game_id='".$real_game_types[$real_game_i]->db_game['game_id']."' ORDER BY option_id ASC;";
			$r = $app->run_query($q);
				...
				if ($num_addr < $real_game_types[$real_game_i]->db_game['min_unallocated_addresses']) {
					...
					try {
						for ($i=0; $i<($real_game_types[$real_game_i]->db_game['min_unallocated_addresses']-$num_addr); $i++) {
							...
							$new_addr_db = $real_game_types[$real_game_i]->create_or_fetch_address($new_addr_str, false, $coin_rpcs[$real_game_i], true, false);
						}
					}
					catch (Exception $e) {
						...
						do {
							...
							$new_addr_db = $real_game_types[$real_game_i]->create_or_fetch_address($temp_address, false, $coin_rpcs[$real_game_i], true, false);

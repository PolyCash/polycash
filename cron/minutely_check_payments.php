...
if ($_REQUEST['key'] != "" && $_REQUEST['key'] == $GLOBALS['cron_key_string']) {
	...
	while ($buyin = mysql_fetch_array($r)) {
		...
		if ($confirm_it) {
			$buyin_game = new Game($buyin['game_id']);
			$buyin_user = new User($buyin['user_id']);
			
			$btc_exchange_rate = $GLOBALS['app']->currency_conversion_rate($buyin['settle_currency_id'], $buyin['pay_currency_id']);
			...
			$coins_in_existence = coins_in_existence($buyin_game->db_game);

				...
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						if ($user_game) {
							...
							$q = "SELECT * FROM currency_invoices ci JOIN currency_addresses ca ON ci.currency_address_id=ca.currency_address_id JOIN currencies c ON ci.settle_currency_id=c.currency_id WHERE ci.game_id=".$game->db_game['game_id']." AND ci.status='confirmed' AND ci.settle_amount > 0;";
							$r = $app->run_query($q);
							...
							$q = "SELECT * FROM game_buyins gb JOIN currency_addresses ca ON gb.currency_address_id=ca.currency_address_id JOIN currencies c ON gb.settle_currency_id=c.currency_id WHERE gb.game_id='".$game->db_game['game_id']."' AND gb.status='confirmed' AND gb.settle_amount > 0;";
							$r = $app->run_query($q);

...
if ($app->running_as_admin()) {
	...
	while ($db_game = $game_r->fetch()) {
		...
		$bet_q = "SELECT gio.game_io_id, gio.colored_amount, gio.option_id, gio.is_coinbase, gio.is_resolved, gio.game_out_index, p.ref_block_id, p.ref_round_id, p.ref_coin_blocks, p.ref_coin_rounds, p.effectiveness_factor, p.effective_destroy_amount, p.destroy_amount, p.votes, p.".$game->db_game['payout_weight']."s_destroyed, p.game_io_id AS parent_game_io_id, io.spend_transaction_id, io.spend_status, ev.*, et.vote_effectiveness_function, et.effectiveness_param1, o.effective_destroy_score AS option_effective_destroy_score, o.unconfirmed_effective_destroy_score, o.unconfirmed_votes, o.name AS option_name, ev.destroy_score AS sum_destroy_score, gio.votes AS votes, o.votes AS option_votes, ug.user_game_id, u.username, u.notification_email FROM addresses a JOIN address_keys ak ON a.address_id=ak.address_id JOIN currency_accounts ca ON ak.account_id=ca.account_id JOIN user_games ug ON ug.account_id=ca.account_id JOIN users u ON ug.user_id=u.user_id JOIN transaction_ios io ON a.address_id=io.address_id JOIN transaction_game_ios gio ON io.io_id=gio.io_id LEFT JOIN transaction_game_ios p ON gio.parent_io_id=p.game_io_id JOIN options o ON gio.option_id=o.option_id JOIN events ev ON o.event_id=ev.event_id LEFT JOIN event_types et ON ev.event_type_id=et.event_type_id JOIN blocks b ON ev.event_payout_block=b.block_id WHERE gio.game_id=".$game->db_game['game_id']." AND gio.is_coinbase=1 AND b.blockchain_id='".$game->blockchain->db_blockchain['blockchain_id']."' AND b.time_created > ".(time()-3600*24)." AND ug.notification_preference='email' AND u.notification_email LIKE '%@%' ORDER BY ug.user_game_id ASC, ev.event_index ASC;";

...
if (empty($GLOBALS['cron_key_string']) || $_REQUEST['key'] == $GLOBALS['cron_key_string']) {
	...
	while ($db_blockchain = $r->fetch()) {
		...
		echo "Checking ".$blockchain->db_blockchain['name']." ".$db_blockchain['first_required_block']." to ".$blockchain->last_block_id()."<br/>\n";
		
		for ($block_id=$db_blockchain['first_required_block']; $block_id<=$blockchain->last_complete_block_id(); $block_id++) {

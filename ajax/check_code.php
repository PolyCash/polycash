...
if ($thisuser) {
	...
	$q = "SELECT * FROM cards c JOIN card_designs d ON c.design_id=d.design_id WHERE c.issuer_card_id=".$app->quote_escape($card_id)." AND d.issuer_id=".$issuer_id.";";
	$r = $app->run_query($q);
			...
			if ($code_hash == $card['secret_hash'] && $num_bruteforce < 100) {
				...
				if ($action == "login") { // Check if the card's valid and try create a gift card account
					if ($password == "" || $password == hash("sha256", "")) {
						...
						$success = $app->try_create_card_account($card, $code, $password);
						if ($success[0]) {
				...
				if ($card['status'] == "redeemed") {
					...
					if ($card_user['password'] == $_REQUEST['password']) {
						...
						if ($card_secret == $_REQUEST['code']) {
							...
							$query .= ", session_key=".$app->quote_escape($session_key).", login_time='".time()."', expire_time='".$expire_time."', ip_address=".$app->quote_escape($_SERVER['REMOTE_ADDR']).";";

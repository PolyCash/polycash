...
if ($email != "") {
	$users_by_email = $app->run_query("SELECT * FROM users WHERE username=".$app->quote_escape($email)." OR notification_email=".$app->quote_escape($email).";");
	...
	if ($users_by_email->rowCount() == 1) {
		...
		$resettoken_q = "INSERT INTO user_resettokens SET user_id='".$db_user['user_id']."', token_key=".$app->quote_escape($token_key).", token2_key=".$app->quote_escape($app->random_string(32)).", create_time='".time()."', expire_time='".(time()+3600*36)."'";
		if ($GLOBALS['pageview_tracking_enabled']) $resettoken_q .= ", request_viewer_id='".$viewer_id."', requester_ip=".$app->quote_escape($_SERVER['REMOTE_ADDR']);
		$resettoken_q .= ";";
		$app->run_query($resettoken_q);
		...
		$res = $app->mail_async($db_user['notification_email'], $GLOBALS['site_name'], "no-reply@".$GLOBALS['site_domain'], $subject, $message, "", "", "");

...
class Match {
	...
	function get_match_round($round_number) {
		...
		if ($r->rowCount() > 0) {
			...
			$r = $app->run_query($q);
			$match_round_id = $app->last_insert_id();
	...
	function add_match_message($message, $from_user_id, $to_user_id, $hide_user_id) {
		$q = "INSERT INTO match_messages SET match_id='".$this->db_match['match_id']."', message=".$app->quote_escape($message);
	...
	function match_body(&$my_membership, $thisuser) {
		...
		if ($this->db_match['status'] == "pending") {
			...
			while ($player = $r->fetch()) {
				...
				$rr = $app->run_query($qq);
				if ($rr->rowCount() > 0) $html .= "Moved submitted";
